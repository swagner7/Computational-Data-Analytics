...
def mykmeans(image, clusterNum):
    ...
    while (len(distortion) == 1 or distortion[-2] - distortion[-1] > 0.0):
        ...
        timing.append(time.time())
        clusterNumber.append(clusterNum)
        print("Distortion value: " + str(round(distortion[-1], 2)) + '\t' + "Distortion delta: " + str(
            round(distortion[-2] - distortion[-1], 2)) + '\t' + "Number of clusters: " + str(clusterNum))
